...
if __name__ == "__main__":
    ...
    hw44 = g2_44 - beta44
    print(f"  n={n44}, q={q44}, d={d44}, γ2={g2_44}, τ={tau44}, β={beta44}")
    print(f"  max_ct0={max_ct0_44},  δ=γ2/max_ct0={g2_44/max_ct0_44:.3f}")
    print()
    print("  WHAT SCALES:")
    print(f"    UseHint oracle: ✓ same mechanism (hint correction per coefficient)")
    print(f"    Interval half-width: {hw44}  (γ2-β, bounds |ct0[i]-center[i]|)")
    print(f"    Constraint structure: m·n linear inequalities on n unknowns")
    print()
    print("  WHAT DOES NOT SCALE (exhaustive):")
    print(f"    Candidate space: 8192^256 = 2^{n44*d44}  ← intractable")
    print()
    print("  IMPORTANT — NTT decomposition does NOT help here:")
    print("    The constraints are in coefficient domain: |M_c · t0 - center|_∞ < hw")
    print("    Each coefficient of c·t0 mixes ALL coefficients of t0 (negacyclic).")
    print("    NTT is pointwise: NTT(c·t0)[j] = NTT(c)[j]·NTT(t0)[j], but the")
    print("    ℓ∞ constraint in coeff domain is NOT equivalent to independent")
    print("    ℓ∞ constraints in NTT domain — no 1-D decomposition available.")
    print()
    print("  PHASE 2 (OPEN PROBLEM):")
    print(f"    The m·N constraints define a feasibility polytope P ⊂ Z^{n44}.")
    print(f"    Unique t0 recovery = CVP/BDD on a {n44}-dimensional lattice.")
    print(f"    δ ≈ {g2_44/max_ct0_44:.3f} — near BDD hardness boundary for LLL/BKZ.")
    print(f"    Status: OPEN — BKZ block size needed vs. current algorithms unknown.")
    print()
    print("  SCOPE OF THIS PoC:")
    print("    ✓ Proves UseHint is an informative interval oracle (not binary)")
    print("    ✓ Exact t0 recovery demonstrated for toy n=4")
    print("    ✗ Does NOT demonstrate t0 recovery for ML-DSA-44 production params")
    print("    ✗ Phase 2 (BDD at δ≈0.60) remains an open research question")
